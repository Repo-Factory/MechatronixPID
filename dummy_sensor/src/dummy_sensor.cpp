...
**/
class DummySensor : public rclcpp::Node
{

public:
    DummySensor() : Node("dummy_sensor") 
    {
        /** 
         * Constructor creates publisher that sends int64 data (epoch time) 
         * and a timer that calls callback every 5 seconds 
        **/
        publisher_ = this->create_publisher<robot_types::msg::Position>("position_data", 10);
        timer_ = this->create_wall_timer(3000ms, std::bind(&DummySensor::timer_callback, this));
    }

private:
    /** 
     * DummySensor() node subclass I made up must declare pointer fields to a timer and publisher object
    **/
    ...
    **/
        ...
        RCLCPP_INFO(this->get_logger(), "Publishing Random Coordinates: ");
...
int main(int argc, char * argv[])
{
  ...
  rclcpp::spin(std::make_shared<DummySensor>()); // simply call constructor of DummySensor Node
  rclcpp::shutdown();
  return 0;
}